...
function runtimeIdentity(env) {
  ...
  if (environment !== "PRODUCTION") throw new Error("Production Worker requires PRODUCTION");
  if (apiUrl !== "/api") throw new Error("Cloudflare frontend API URL must be /api");
...
function runtimeConfig(env) {
  const identity = runtimeIdentity(env);
  return new Response(
    `window.__FASTLINK_RUNTIME__ = Object.freeze(${JSON.stringify({
