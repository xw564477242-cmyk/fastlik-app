...
export function WalletExperience(props:Props){
 ...
 return <div className="consumer-wallet">
  <div className="consumer-scroll">
   ...
   {page==='assets'&&<Assets accounts={props.accounts} summary={props.summary} transactions={props.transactions} onConvert={()=>go('convert')} onDigital={()=>go('digital')} onCards={()=>go('cards')}/>} 
...
function Assets({accounts,summary,transactions,onConvert,onDigital,onCards}:{accounts:WalletAccountRecord[];summary:WalletBalanceSummary|null;transactions:WalletTransactionPage|null;onConvert:()=>void;onDigital:()=>void;onCards:()=>void}){const[settlementCurrency,setSettlementCurrency]=useState<'CNY'|'USD'|'SGD'>('USD');return <main className="consumer-page"><section className="fiat-total"><div className="balance-label"><span>总法币资产</span><label>结算单位<select value={settlementCurrency} onChange={event=>setSettlementCurrency(event.target.value as 'CNY'|'USD'|'SGD')}><option>CNY</option><option>USD</option><option>SGD</option></select></label></div><strong>—</strong><p>以 {settlementCurrency} 结算 · Backend 第三方汇率汇总待接入</p></section><div className="asset-categories compact"><button className="active"><Banknote/><span><b>法币资产</b><small>CNY · USD · SGD · VND</small></span><ChevronRight/></button><button onClick={onDigital}><Bitcoin/><span><b>数字货币资产</b><small>USDT · USDC · ETH</small></span><ChevronRight/></button><button onClick={onCards}><CreditCard/><span><b>卡片资产</b><small>虚拟卡 · 实体卡</small></span><ChevronRight/></button></div><section className="consumer-section"><div className="consumer-section-title"><h2>法币账户</h2><span>点击兑换</span></div>{fiatAssetCodes.map(code=>{const account=accounts.find(item=>item.assetCode===code);const balance=summary?.items.find(item=>item.assetCode===code);return <button className={`asset-row currency-row ${account?'':'unavailable-asset'}`} key={code} onClick={onConvert}><span>{code.slice(0,2)}</span><div><b>{code}</b><small>{account?`${account.status} · ${account.id}`:'Dev Backend 尚未返回此币种账户'}</small></div><strong>{balance?.availableBalance??account?.availableBalance??'未返回'}</strong><ChevronRight/></button>})}</section><section className="consumer-section fiat-ledger"><div className="consumer-section-title"><h2>法币资产账本</h2><span>资金进出明细</span></div>{transactions?.items.length?<div className="operation-list">{transactions.items.map(row=><article className="operation-row" key={row.id}><span>{row.direction==='INCOMING'?<ArrowDownToLine/>:<ArrowUpFromLine/>}</span><div><b>{row.type.replaceAll('_',' ')}</b><small>{row.status} · {new Date(row.createdAt).toLocaleString()}</small></div><strong className={row.direction==='INCOMING'?'incoming':''}>{row.direction==='INCOMING'?'+':'-'}{row.amount} {row.assetCode}</strong></article>)}</div>:<div className="consumer-empty tall"><History/><b>暂无法币进出明细</b><small>Backend 没有返回充值、支付或转账账本记录。</small></div>}</section></main>}
