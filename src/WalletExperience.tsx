...
import {ArrowDownToLine,ArrowLeft,ArrowLeftRight,ArrowUpFromLine,BadgeCheck,Banknote,Bell,Bitcoin,Building2,ChevronRight,CircleUserRound,CreditCard,Eye,EyeOff,Globe2,Headphones,HelpCircle,History,Home,Layers,LockKeyhole,LogOut,Plus,QrCode,RefreshCw,ScanLine,Send,Settings,ShieldCheck,Smartphone,User,WalletCards} from 'lucide-react'
...
export function WalletExperience(props:Props){
 ...
 return <div className="consumer-wallet">
  <div className="consumer-scroll">
   ...
   {page==='digital'&&<PendingFlow kind="digital"/>}
   {page==='cards'&&<Cards cards={props.cards} selected={props.selectedCard} balance={props.cardBalance} limits={props.cardLimits} transactions={props.cardTransactions} onSelect={props.onSelectCard}/>} 
...
function Assets({accounts,summary,onDigital,onCards}:{accounts:WalletAccountRecord[];summary:WalletBalanceSummary|null;onDigital:()=>void;onCards:()=>void}){return <main className="consumer-page"><section className="consumer-hero"><WalletCards/><span>钱包账户</span><strong>{accounts.length}</strong><small>所有金额均来自 FastLink Dev，只读展示</small></section><div className="asset-categories"><button className="active"><Banknote/><span><b>法币钱包</b><small>{accounts.length} 个账户</small></span><ChevronRight/></button><button onClick={onDigital}><Bitcoin/><span><b>数字资产</b><small>待开发</small></span><ChevronRight/></button><button onClick={onCards}><CreditCard/><span><b>卡片资产</b><small>查看卡片中心</small></span><ChevronRight/></button></div><section className="consumer-section"><div className="consumer-section-title"><h2>法币资产明细</h2><span>只读</span></div>{accounts.map(account=>{const balance=summary?.items.find(item=>item.assetCode===account.assetCode);return <article className="asset-row" key={account.id}><span>{account.assetCode.slice(0,2)}</span><div><b>{account.assetCode}</b><small>{account.status} · {account.id}</small></div><strong>{balance?.availableBalance??account.availableBalance}</strong></article>})}{accounts.length===0&&<div className="consumer-empty">Backend 没有返回钱包账户</div>}</section></main>}
