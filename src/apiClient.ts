...
export const walletRuntime=Object.freeze({apiUrl,environment,buildSha:(import.meta.env.VITE_FASTLINK_BUILD_SHA as string|undefined)||'unknown'})
...
async function request<T>(path:string,token:string,method='GET',body?:unknown,idempotencyKey?:string):Promise<T>{
 const id=trace();const response=await fetch(`${apiUrl}${path}`,{method,cache:'no-store',headers:{Accept:'application/json','X-Trace-Id':id,Authorization:`Bearer ${token}`,...(body?{'Content-Type':'application/json'}:{}),...(idempotencyKey?{'Idempotency-Key':idempotencyKey}:{})},...(body?{body:JSON.stringify(body)}:{})});const returned=response.headers.get('x-trace-id')||id;if(!response.ok){let message=`HTTP ${response.status}`;try{const payload=await response.json();message=Array.isArray(payload.message)?payload.message.join(', '):(payload.message||message)}catch{}throw new WalletApiError(response.status,returned,`${message} · HTTP ${response.status} · Trace ${returned}`)}return response.json() as Promise<T>
}
