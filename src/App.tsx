...
export default function App(){
 ...
 const refresh=async()=>{if(!session)return;abortCardDetailRequest();setBusy(true);setError('');try{await acceptSession(await walletApi.refresh())}catch(value){if(sessionFailureRequiresClear(value))handleSessionInvalidation(value);else setError(describe(value))}finally{setBusy(false)}}
 ...
 const consumerExperience=session?<WalletExperience session={session} accounts={accounts} summary={walletBalanceSummary} operations={walletOperations} transactions={walletTransactions} cards={cards} selectedCard={selectedCard} cardBalance={cardBalance} cardLimits={cardLimits} cardTransactions={cardTransactions} loading={busy} errors={[error,walletBalanceSummaryError,walletError,walletHistoryError,walletOperationsError,cardListError,cardRefreshError,cardBalanceError,cardTransactionError].filter(Boolean)} buildSha={walletRuntime.buildSha} onRefresh={()=>void refresh()} onSelectCard={card=>void selectCard(card)} onLogout={()=>void logout()}/>:null
 if(consumerExperience)return consumerExperience
