...
import {CardTransactionRecord,cardTransactionLifecycleType,mergeCardTransactionPages} from './cardTransactions'
...
export default function App(){
 ...
 const replaceCardTransactions=(rows:readonly CardTransactionRecord[])=>{const next=[...rows];cardTransactionsRef.current=next;setCardTransactions(next);replaceSelectedCardTransactionDetail(reconcileCardTransactionDetailSelection(selectedCardTransactionDetailRef.current,cardScope.current,selectedCardRef.current?.id??null,next,walletRequestMounted.current))}
 const clearCardTransactions=()=>{cardTransactionRequestSequence.current+=1;cardTransactionTarget.current=null;replaceCardTransactions([]);setCardTransactionNextCursor(null);setCardTransactionLoadingMore(false);setCardTransactionError('')}
 ...
 const loadCard=async(card:CardRecord,expectedScope=cardScope.current,callbacks?:{onError?:(value:unknown)=>void;onSettled?:()=>void})=>{
  ...
  try{
   ...
   replaceCardTransactions(snapshot.transactions.transactions)
