...
function WalletHome({hidden,setHidden,notify,go}:{hidden:boolean;setHidden:(v:boolean)=>void;notify:(s:string)=>void;go:(p:Page)=>void}) {
  return <div className="content"><section className="mock-disclosure"><b>MOCK DATA</b><span>本页面未连接 Thredd UAT；所有余额和交易均为本地演示数据。</span></section><section className="balance-hero"><div><span>TOTAL ASSETS <button onClick={()=>setHidden(!hidden)}>{hidden?<Eye/>:<EyeOff/>}</button></span><strong>{hidden?'••••••':'$21,319.34'}</strong><p><b>+2.84%</b> in the last 30 days</p></div><div className="balance-split"><button><i className="crypto"/><span>Digital assets</span><b>{hidden?'••••':'$14,630.50'}</b></button><button><i className="fiat"/><span>Fiat wallet</span><b>{hidden?'••••':'$6,688.84'}</b></button><button onClick={()=>go('cards')}><i className="card"/><span>Card balance</span><b>{hidden?'••••':'$4,276.00'}</b></button></div></section><section className="quick-actions">{[[Plus,'Deposit'],[ArrowUpRight,'Withdraw'],[Send,'Transfer'],[ScanLine,'Scan & Pay']].map(([Icon,label])=><button key={label as string} onClick={()=>notify(`${label} flow opened in MOCK`)}><span><Icon/></span>{label as string}</button>)}</section><div className="grid-two"><section className="panel"><div className="panel-title"><div><h2>My assets</h2><p>Digital assets and fiat accounts</p></div><button>Manage <ChevronRight/></button></div><div className="asset-list">{assets.map(asset=><button key={asset.code} onClick={()=>notify(`${asset.code} MOCK account opened`)}><span className="coin" style={{background:asset.color}}>{asset.code[0]}</span><div><b>{asset.code}</b><small>{asset.name} · {asset.network}</small></div><section><b>{hidden?'••••':asset.amount}</b><small>{hidden?'••••':asset.fiat}</small></section><ChevronRight/></button>)}</div></section><section className="panel card-preview"><div className="panel-title"><div><h2>FastLink Card</h2><p>Ready for global spending</p></div><button onClick={()=>go('cards')}>Card Center <ChevronRight/></button></div><MiniCard/><div className="card-stats"><p><span>Available</span><b>$4,276.00</b></p><p><span>This month</span><b>$1,284.60</b></p></div></section></div><section className="panel transactions"><div className="panel-title"><div><h2>Recent activity</h2><p>Wallet, exchange and card transactions</p></div><button>View all <ChevronRight/></button></div>{transactions.map(({icon:Icon,...tx})=><div className="transaction" key={tx.name+tx.time}><span><Icon/></span><div><b>{tx.name}</b><small>{tx.meta}</small></div><section><b className={tx.positive?'positive':''}>{tx.amount}</b><small>{tx.time}</small></section></div>)}</section></div>
}
