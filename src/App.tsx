...
export default function App(){
 ...
 const resetTransferInputs=()=>{walletTransferInputGeneration.current+=1;destinationAccountIdRef.current='';transferAmountRef.current='';setDestinationAccountId('');setTransferAmount('')}
 const invalidateTransferForInputChange=()=>{walletTransferRequestSequence.current+=1;walletTransferTarget.current=null;walletTransferSubmitGate.current.activeRequestId=null;walletTransferStatusRequestSequence.current+=1;walletTransferStatusTarget.current=null;walletTransferStatusInFlight.current=false;setTransferBusy(false);setTransferReceipt(null);setTransferStatusBusy(false);setTransferStatusRefreshCount(0);setTransferError('')}
 const updateTransferDestination=(value:string)=>{if(value===destinationAccountIdRef.current)return;walletTransferInputGeneration.current+=1;destinationAccountIdRef.current=value;invalidateTransferForInputChange();setDestinationAccountId(value)}
 const updateTransferAmount=(value:string)=>{if(value===transferAmountRef.current)return;walletTransferInputGeneration.current+=1;transferAmountRef.current=value;invalidateTransferForInputChange();setTransferAmount(value)}
 const clearAcceptedTransferAmount=()=>{transferAmountRef.current='';walletTransferInputGeneration.current+=1;setTransferAmount('')}
 ...
 useEffect(()=>{walletRequestMounted.current=true;return()=>{walletRequestMounted.current=false;selectedCardTransactionDetailRef.current=null;cardRequestSequence.current+=1;abortCardListRequest();cardListCursorTarget.current=null;cardDetailRequestSequence.current+=1;abortCardDetailRequest();abortCardTransactionRequest();abortCardTransactionDetailRequest();abortCardTimelineRequest();cardTransactionRequestSequence.current+=1;cardTransactionDetailRequestSequence.current+=1;cardTimelineRequestSequence.current+=1;cardTransactionTarget.current=null;cardTransactionCursorTarget.current=null;cardTimelineTarget.current=null;cardTimelineCursorTarget.current=null;walletOperationRequestSequence.current+=1;walletOperationDetailRequestSequence.current+=1;walletOperationInFlight.current=false;abortWalletOperationRequest();abortWalletOperationDetailRequest();walletHistoryRequestSequence.current+=1;walletTransactionDetailRequestSequence.current+=1;walletTransferRequestSequence.current+=1;walletTransferTarget.current=null;walletTransferSubmitGate.current.activeRequestId=null;walletTransferStatusRequestSequence.current+=1;walletTransferStatusTarget.current=null;walletTransferStatusInFlight.current=false;walletTransferInputGeneration.current+=1;cardLimitsUpdateRequestSequence.current+=1;cardLimitsUpdateSubmitGate.current.activeRequestId=null;cardLimitsUpdateInFlight.current=false;cardActionRequestSequence.current+=1;cardActionTarget.current=null;cardStatusSubmitGate.current.activeRequestId=null;cardStatusInFlight.current=false;cardReplacementRequestSequence.current+=1;cardReplacementTarget.current=null;cardReplacementSubmitGate.current.activeRequestId=null;cardReplacementInFlight.current=false;cardRenewalRequestSequence.current+=1;cardRenewalTarget.current=null;cardRenewalSubmitGate.current.activeRequestId=null;cardRenewalInFlight.current=false;abortWalletAccountBalanceRequest();abortWalletHistoryRequest();abortWalletTransactionDetailRequest();abortWalletBalanceSummaryRequest()}},[])
 ...
 const replaceSelectedCard=async(event:FormEvent)=>{
  ...
  try{
   ...
   setCardListError('')
   setBusy(true)
   await loadCard(commit.selectedCard,request.scopeKey,activeSession,detailCallbacks)
 ...
 const transfer=async()=>{
  ...
  try{
   ...
   if(!refreshedAccount){clearAcceptedTransferAmount();walletAccountRequestSequence.current+=1;walletAccountTarget.current=null;replaceSelectedAccount(null);clearWalletBalanceSummary();replaceAccountBalance(null);clearWalletTransactions();setTransferError('Transfer accepted, but the source account is no longer available in this session.');return}
   ...
   setTransferError(receipt?'Transfer accepted, but current balances could not be refreshed.':describeWalletTransfer())
  }finally{
   const current=isCurrent()
 ...
 return <main style={{maxWidth:980,margin:'40px auto',padding:24,fontFamily:'Inter,system-ui'}}>
  ...
  {session&&<div className="wallet-grid">
   <KycStatusPanel session={session} runtimeEnvironment={walletRuntime.environment}/>
   ...
   <section className="panel">
    ...
    {selectedAccount&&<>
     ...
     <ConsumerTransferFlow title="Internal transfer" accounts={accounts} source={selectedAccount} destinationAccountId={destinationAccountId} amount={transferAmount} busy={transferBusy} loading={busy||walletLoading||!accountBalance} error={transferError} receipt={transferReceipt} statusBusy={transferStatusBusy} statusRefreshCount={transferStatusRefreshCount} statusRefreshLimit={WALLET_TRANSFER_STATUS_REFRESH_LIMIT} onDestinationChange={updateTransferDestination} onAmountChange={updateTransferAmount} onSubmit={transfer} onRefreshStatus={refreshTransferStatus}/>
