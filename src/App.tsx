...
import {AuthenticatedBusinessWriteBoundary,ReadOnlyUatCapabilityMarker} from './AuthenticatedBusinessWriteBoundary'
import {ReadOnlyUatAuthenticatedView} from './ReadOnlyUatAuthenticatedView'
import {isReadOnlyUatSession} from './interactionMode'
import {walletGateway} from './gateway/index'
...
export default function App(){
 ...
 const[sessionInitializationReady,setSessionInitializationReady]=useState(false)
 ...
 const acceptSession=async(current:WalletSession)=>{
  setSessionInitializationReady(false)
  ...
  if(isCurrent())setSessionInitializationReady(true)
 }
 ...
 const authenticatedBoundaryProps=session?{interactionMode:walletRuntime.interactionMode,runtimeEnvironment:walletRuntime.environment,sessionEnvironment:session.environment}:null
 const readOnlyUatAuthenticated=Boolean(session&&isReadOnlyUatSession(walletRuntime.environment,session.environment,walletRuntime.interactionMode))
 ...
 return <main style={{maxWidth:980,margin:'40px auto',padding:24,fontFamily:'Inter,system-ui'}}>
  <header><div><h1>FastLink Wallet</h1><p>Environment: <b>{walletRuntime.environment}</b> · Build: <b>{walletRuntime.buildSha}</b></p><p>API: <code>{walletRuntime.apiUrl}</code></p></div>{session&&!readOnlyUatAuthenticated&&<div className="session-actions"><button onClick={refresh} disabled={sessionActionBusy||transferBusy||(transferRetryPending&&!transferRetryExhausted)||cardReplacing||cardRenewing}><RefreshCw/> Refresh session</button><button onClick={logout} disabled={sessionActionBusy||transferBusy||transferRetryPending||cardReplacing||cardRenewing}><LogOut/> Sign out</button></div>}</header>
  {!session&&<section className="panel auth-panel"><ShieldCheck/><h2>{walletRuntime.interactionMode==='READ_ONLY_UAT'||mode==='login'?'Sign in to FastLink':'Create your FastLink account'}</h2><p>Authentication is handled by the FastLink Backend using a secure HttpOnly cookie. Browser storage and pasted bearer tokens are disabled.</p><form onSubmit={authenticate}><label>Workspace<input value={tenantId} onChange={event=>setTenantId(event.target.value)} placeholder="Tenant ID or slug" autoComplete="organization" required/></label><label>Email<input type="email" value={email} onChange={event=>setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" required/></label><label>Password<input type="password" value={password} onChange={event=>setPassword(event.target.value)} placeholder="Minimum 12 characters" autoComplete={walletRuntime.interactionMode==='READ_ONLY_UAT'||mode==='login'?'current-password':'new-password'} minLength={12} required/></label><button disabled={busy||!tenantId.trim()||!email.trim()||password.length<12}>{busy?'Please wait…':walletRuntime.interactionMode==='READ_ONLY_UAT'||mode==='login'?'Sign in':'Register'}</button></form>{walletRuntime.interactionMode==='FULL'&&<button className="mode-switch" onClick={()=>{setMode(current=>current==='login'?'register':'login');setError('')}}>{mode==='login'?'New to FastLink? Register':'Already registered? Sign in'}</button>}</section>}
  {error&&<section className="panel"><h3>API unavailable</h3><p>{error}</p><p>Unavailable · no stale data displayed.</p></section>}
  {session&&readOnlyUatAuthenticated&&<WalletRenderBoundary resetKey={sessionScope(session)} title="Read-only Wallet view unavailable" message="The restricted authenticated Wallet view could not be rendered safely." retryEnabled={false}><ReadOnlyUatAuthenticatedView interactionMode={walletRuntime.interactionMode} runtimeEnvironment={walletRuntime.environment} session={session} summary={walletBalanceSummary} summaryLoading={walletBalanceSummaryLoading} summaryUnavailable={Boolean(walletBalanceSummaryError)} accounts={accounts} accountsUnavailable={Boolean(walletError)} cards={cards} cardsUnavailable={Boolean(cardListError)} operations={walletOperations} operationsLoading={walletOperationsLoading} operationsUnavailable={Boolean(walletOperationsError)} catalogReadEnabled={sessionInitializationReady} cardProductsGateway={walletGateway}/></WalletRenderBoundary>}
  {session&&!readOnlyUatAuthenticated&&<WalletRenderBoundary resetKey={sessionScope(session)} title="Wallet view unavailable" message="The authenticated Wallet view could not be rendered safely."><div className="wallet-grid">
   <ReadOnlyUatCapabilityMarker {...authenticatedBoundaryProps!}/>
   ...
   <Phase2WalletPanel accounts={accounts} selectedCardId={selectedCard?.id??null} sessionEnvironment={session.environment} sessionKey={sessionScope(session)} catalogReadEnabled={sessionInitializationReady}/>
   ...
   <AuthenticatedBusinessWriteBoundary {...authenticatedBoundaryProps!}><FxQuotePreview session={session} accounts={accounts}/></AuthenticatedBusinessWriteBoundary>
   <section className="panel">
    ...
    {selectedAccount&&<>
     ...
     <AuthenticatedBusinessWriteBoundary {...authenticatedBoundaryProps!}><ConsumerTransferFlow title="Internal transfer" accounts={accounts} source={selectedAccount} destinationAccountId={destinationAccountId} amount={transferAmount} busy={transferBusy} retryPending={transferRetryPending} retryExhausted={transferRetryExhausted} loading={busy||walletLoading||!accountBalance} error={transferError} receipt={transferReceipt} statusBusy={transferStatusBusy} statusRefreshCount={transferStatusRefreshCount} statusRefreshLimit={WALLET_TRANSFER_STATUS_REFRESH_LIMIT} onDestinationChange={updateTransferDestination} onAmountChange={updateTransferAmount} onSubmit={transfer} onRefreshStatus={refreshTransferStatus}/></AuthenticatedBusinessWriteBoundary>
   ...
   <section id="wallet-cards" className="panel">
    ...
    {selectedCard&&<>
     ...
     <AuthenticatedBusinessWriteBoundary {...authenticatedBoundaryProps!}>
   ...
   {selectedCard&&<section className="panel card-limits-record">
    <h2>Card limits · public contract</h2>
    {cardLimitsLoading&&<p>Loading Card limits…</p>}
    {cardLimitsError&&<div className="inline-error">{cardLimitsError} · No unvalidated or cross-card limits displayed.</div>}
    {cardLimits&&<>
     <div><span>Single transaction</span><b>{cardLimits.singleTransactionMinor??'Not set'}</b></div>
     <div><span>Daily spend</span><b>{cardLimits.dailySpendMinor??'Not set'}</b></div>
     <div><span>Monthly spend</span><b>{cardLimits.monthlySpendMinor??'Not set'}</b></div>
     <div><span>Daily ATM</span><b>{cardLimits.dailyAtmMinor??'Not set'}</b></div>
     <small>{cardLimits.updatedAt?`Updated ${new Date(cardLimits.updatedAt).toLocaleString()}`:'No persisted limit update'}</small>
     <AuthenticatedBusinessWriteBoundary {...authenticatedBoundaryProps!}>
      {limitsUpdateDecision?.allowed&&<form className="card-limits-update-form" onSubmit={submitSelectedCardLimits}><h3>Update selected Card limits · {session?.environment}</h3>{CARD_LIMIT_UPDATE_FIELDS.map(field=><label key={field}>{field}<input value={cardLimitsUpdateDraftState[field]} onChange={event=>updateCardLimitsDraftValue(field,event.target.value)} inputMode="numeric" pattern="^(?:0|[1-9][0-9]*)$" maxLength={13} placeholder="Not set" disabled={cardLimitsUpdating} /></label>)}<button disabled={busy||cardLimitsUpdating||virtualCardCreating||cardReplacing||cardRenewing||!CARD_LIMIT_UPDATE_FIELDS.some(field=>cardLimitsUpdateDraftState[field]!=='')}>{cardLimitsUpdating?'Updating once…':'Apply limits once'}</button>{cardLimitsUpdateError&&<div className="inline-error">{cardLimitsUpdateError} · No Provider or internal response fields displayed.</div>}<p className="card-action-note">0–{CARD_LIMIT_UPDATE_MAX_MINOR.toLocaleString()} minor units · single ≤ daily ≤ monthly. One manual submission, one new UUIDv4 Idempotency-Key, at most one POST, no automatic retries.</p></form>}
     </AuthenticatedBusinessWriteBoundary>
    </>}
   </section>}
  </div></WalletRenderBoundary>}
 </main>
}
