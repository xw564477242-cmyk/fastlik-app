...
export default function App(){
 ...
 const invalidateCardDetail=()=>{abortCardDetailRequest();abortCardTransactionRequest();cardDetailRequestSequence.current+=1;cardDetailTarget.current=null;cardSnapshotTarget.current=null;cardBalanceRequestSequence.current+=1;cardBalanceTarget.current=null;cardLimitsRequestSequence.current+=1;cardLimitsTarget.current=null;cardLimitsUpdateRequestSequence.current+=1;cardLimitsUpdateSubmitGate.current.activeRequestId=null;cardLimitsUpdateInFlight.current=false;clearCardStatusAction();cardTransactionRequestSequence.current+=1;cardTransactionTarget.current=null;cardTransactionCursorTarget.current=null}
 ...
 const replaceCardTransactionHistory=(history:CardTransactionHistoryState|null)=>{cardTransactionHistoryRef.current=history;const next=history?[...history.transactions]:[];cardTransactionsRef.current=next;setCardTransactions(next);setCardTransactionNextCursor(history?.nextCursor??null);replaceSelectedCardTransactionDetail(reconcileCardTransactionDetailSelection(selectedCardTransactionDetailRef.current,cardScope.current,selectedCardRef.current?.id??null,next,walletRequestMounted.current))}
 const clearCardTransactions=()=>{abortCardTransactionRequest();cardTransactionRequestSequence.current+=1;cardTransactionTarget.current=null;cardTransactionCursorTarget.current=null;replaceCardTransactionHistory(null);setCardTransactionLoadingMore(false);setCardTransactionError('')}
