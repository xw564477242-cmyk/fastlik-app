...
assert(apiClient.includes("readFxQuote(fxQuoteTransport,session,walletRuntime.environment,input)"), "FX quote must pass through the strict session-gated consumer");
assert(apiClient.includes("fxQuoteTransport=({path,method,body}:FxQuoteTransportRequest)=>request<string>(path,method,body,undefined,'text')"), "FX quote must use the cookie and CSRF aware same-origin transport");
...
assert(fxQuotePreview.includes("mounted.current=false") && fxQuotePreview.includes("scopeRef.current===expectedScope") && fxQuotePreview.includes("submitGate.current.activeRequestId===requestId"), "FX quote UI must reject stale, cross-session and unmounted completions");
