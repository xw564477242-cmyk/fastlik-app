...
assert(app.includes("if(confirmedActivation)") && app.includes("cardStatusRetryRequest.current=null") && app.includes("Associated Card data was safely cleared") && app.includes("cardDetailTarget.current=null"), "A confirmed activation with failed associated reads must retain ACTIVE, drop retry state and invalidate all dependent Card state");
