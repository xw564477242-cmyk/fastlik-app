...
assert(app.includes("createCardActivationCommit(confirmedActivation,snapshot)") && app.includes("readCardDetailRefresh({"), "Confirmed activation must refresh Card, balance, limits, transactions and timeline before one commit");
assert(app.includes("if(confirmedActivation)") && app.includes("Associated Card data was safely cleared") && app.includes("cardDetailTarget.current=null"), "A confirmed activation with failed associated reads must retain ACTIVE while invalidating all dependent Card state");
...
assert(app.includes("sessionFailureRequiresClear(value)){walletTransferRetryRequest.current=null;handleSessionInvalidation(value);return}") && app.includes("if(!isCurrent())return;if(sessionFailureRequiresClear(value)"), "Only a current transfer or status failure may invalidate the active session");
assert(app.includes("'Transfer accepted, but current balances could not be refreshed.'") && app.includes("'Transfer result uncertain. Retry once to reuse the same Idempotency-Key safely.'"), "Wallet transfer failures must distinguish accepted receipts from safely retryable uncertain results without Provider details");
assert(app.includes("const refresh=async()=>{if(!session||sessionActionBusy||transferBusy||walletTransferRetryRequest.current)return") && app.includes("disabled={sessionActionBusy||transferBusy||transferRetryPending||cardReplacing||cardRenewing}"), "Session controls must fail closed while a transfer is settling or has an uncertain result");
