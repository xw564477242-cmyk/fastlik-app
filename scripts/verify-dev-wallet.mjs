...
assert(app.includes("const refresh=async()=>{if(!session||sessionActionBusy||transferBusy)return") && app.includes("disabled={sessionActionBusy||transferBusy||cardReplacing||cardRenewing}"), "Session refresh must fail closed while an accepted transfer is still settling");
