...
const httpTransport = readFileSync(join(root, "src/gateway/httpTransport.ts"), "utf8");
const gatewayIndex = readFileSync(join(root, "src/gateway/index.ts"), "utf8");
...
const walletAccountTransactionsIntegrationTests = readFileSync(join(root, "tests/walletAccountTransactions.integration.test.ts"), "utf8");
...
assert(httpTransport.includes("fastlink_csrf"), "Wallet API must use the CSRF cookie/header contract");
assert(!httpTransport.includes("Authorization"), "Wallet API must not send a Bearer token");
assert(!httpTransport.includes("localStorage"), "Wallet authentication must not use localStorage");
assert(!httpTransport.includes("sessionStorage"), "Wallet authentication must not use sessionStorage");
assert(!httpTransport.includes("supabase"), "Wallet authentication must not use Supabase frontend sessions");
assert(gatewayIndex.includes("Production Wallet builds forbid MockWalletGateway"), "Production must fail closed when MockWalletGateway is selected");
assert(!httpTransport.includes("exquisite-surprise-production"), "Wallet source must not hard-code the Production Backend");
assert(httpTransport.includes("VITE_FASTLINK_API_URL"), "Wallet API must require an explicit build API URL");
assert(httpTransport.includes("VITE_FASTLINK_ENVIRONMENT"), "Wallet must require an explicit environment");
assert(httpTransport.includes("VITE_FASTLINK_BUILD_SHA"), "Wallet must expose the build SHA");
...
assert(apiClient.includes("if(!readCsrfToken())return Promise.reject") && apiClient.includes("'caller')"), "Card status mutation must fail closed without CSRF and leave exact 401 invalidation to its request owner");
...
assert(app.includes("await runWalletTransferPostChain({") && app.includes("walletApi.walletTransferBalance") && app.includes("walletApi.walletTransferAccountTransactions"), "The production App must consume the atomic transfer post-chain through caller-owned account readers");
assert(walletTransferPostChain.includes("historiesMatchConfirmedOperation") && walletTransferPostChain.includes('entry.operationId === receipt.id') && walletTransferPostChain.includes('"OUTGOING"') && walletTransferPostChain.includes('"INCOMING"'), "The transfer post-chain must associate one source debit and destination credit with the same confirmed operation");
assert(walletTransferPostChainIntegrationTests.includes("one POST is persisted-confirmed") && walletTransferPostChainIntegrationTests.includes("TRANSACTIONS:${value.source.id}") && walletTransferPostChainIntegrationTests.includes("late account-history success, error, and 401"), "Executable SANDBOX/TEST evidence must prove one POST, two account histories and zero stale writes");
assert(walletTransactions.includes("walletAccountTransactionPath") && walletTransactions.includes("previous.accountId !== accountId") && walletTransactions.includes("operationId"), "Wallet account history must bind the opaque cursor to one owned account and expose only the public operation association");
assert(walletAccountTransactionsIntegrationTests.includes("rejects cross-account reuse before transport") && walletAccountTransactionsIntegrationTests.includes("exact public operation association"), "Account-history evidence must prove cursor/account binding and exact operation parsing");
...
assert(app.includes("Transfer is confirmed, but the complete source and destination account-history refresh failed") && app.includes("Retry manually once to reuse the exact Idempotency-Key"), "Wallet transfer failures must distinguish confirmed safe invalidation from exact-key manual retry");
...
assert(apiClient.includes("walletTransactionTransport=({path,method,signal}") && httpTransport.includes("input.signal?.addEventListener('abort', cancel, {once: true})"), "Wallet transaction fetches must compose timeout and caller cancellation signals");
...
assert(app.includes("walletApi.walletAccountTransactions(activeSession,account,filters,null,historyController.signal)") && app.includes("walletApi.walletAccountTransactions(session,selectedAccount,filters,previous,historyController.signal)"), "Wallet account list and next-page requests must carry their active cancellation signal");
