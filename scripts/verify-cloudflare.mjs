...
const interactionMode = read("src/interactionMode.ts");
const businessWriteBoundary = read("src/AuthenticatedBusinessWriteBoundary.ts");
const readOnlyAuthenticatedView = read("src/ReadOnlyUatAuthenticatedView.ts");
...
assert(publicRuntimeConfig.includes("interactionMode"), "Static runtime config fallback must preserve the interaction-mode contract");
...
assert(httpTransport.includes("readOnlyUatRequestAllowed(input.path, method)") && httpTransport.indexOf("readOnlyUatRequestAllowed(input.path, method)") < httpTransport.indexOf("await fetch("), "READ_ONLY_UAT must block business writes before browser transport");
assert(interactionMode.includes("'/v1/auth/login'") && interactionMode.includes("'/v1/auth/refresh'") && interactionMode.includes("'/v1/auth/logout'") && !interactionMode.includes("'/v1/auth/register'"), "READ_ONLY_UAT must use the exact authentication lifecycle POST allowlist");
assert(interactionMode.includes("READ_ONLY_UAT is allowed only in TEST"), "READ_ONLY_UAT must fail closed outside TEST");
assert(businessWriteBoundary.includes("data-business-write-surface") && businessWriteBoundary.includes("READ_ONLY_UAT_MARKER"), "The authenticated read-only profile must expose an exact capability marker with no write surface");
assert(app.includes("ReadOnlyUatCapabilityMarker") && app.includes("AuthenticatedBusinessWriteBoundary"), "The production authenticated tree must mount the capability marker and business-write boundary");
assert(app.includes("session&&readOnlyUatAuthenticated&&<WalletRenderBoundary") && app.includes("<ReadOnlyUatAuthenticatedView") && app.includes("session&&!readOnlyUatAuthenticated&&<WalletRenderBoundary"), "App must route READ_ONLY_UAT away from the complete interactive authenticated tree");
assert(app.includes('session&&!readOnlyUatAuthenticated&&<div className="session-actions"'), "READ_ONLY_UAT must not mount authenticated header actions");
assert(readOnlyAuthenticatedView.includes("Phase2TestCardProductsPanel") && readOnlyAuthenticatedView.includes("data-readonly-uat-authenticated-view") && !readOnlyAuthenticatedView.includes("'button'") && !readOnlyAuthenticatedView.includes("'form'") && !readOnlyAuthenticatedView.includes("'select'") && !readOnlyAuthenticatedView.includes("'input'"), "The dedicated restricted view must mount the real catalogue panel with no interactive control element");
assert(app.includes("catalogReadEnabled={sessionInitializationReady}") && app.indexOf("setSessionInitializationReady(true)") > app.indexOf("await runBoundedWalletInitialization"), "The TEST Card catalogue must wait for authenticated Wallet initialization");
...
assert(worker.includes("readOnlyUatProxyRequestAllowed(publicUrl.pathname, request.method, publicUrl.search)") && worker.indexOf("readOnlyUatProxyRequestAllowed(publicUrl.pathname, request.method, publicUrl.search)") < worker.indexOf("const response = await fetch("), "READ_ONLY_UAT Worker must deny business mutations and auth POST queries before Backend transport");
assert(worker.includes("READ_ONLY_UAT_WRITE_BLOCKED") && worker.includes("interactionMode: identity.interactionMode"), "Worker health and runtime identity must expose the fixed READ_ONLY_UAT capability");
...
assert(testConfig.includes('"FASTLINK_INTERACTION_MODE": "FULL"'), "Shared Test Worker must preserve FULL interaction semantics");
...
assert(devConfig.includes('"FASTLINK_INTERACTION_MODE": "FULL"'), "Shared Dev Worker must preserve FULL interaction semantics");
